...
public final class Pend<T, E extends Exception> {
    private final AtomicReference<Result<T,E>> result;
    private final AtomicReference<Consumer<Result<T,E>>> callback;

    public Pend() {
        result = new AtomicReference<>(null);
        callback = new AtomicReference<>(null);
    }
